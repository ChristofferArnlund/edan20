...
def predict(test_sentences, feature_names, f_out):
    for test_sentence in test_sentences:
        ...
        for x in X_test_dict:
            ...
            #previous ns
            c_n2 = c_n1
            c_n1 = dict_classes[y_test_predicted[0]]
...
def encode_classes(y_classes):
    ...
    classes = sorted(list(set(y_classes)))
    ...
    y_encoded = [inv_dict_classes[i] for i in y_classes]
    return y_encoded, dict_classes, inv_dict_classes
...
if __name__ == '__main__':
    ...
    feature_names = ['word_n2', 'word_n1', 'word', 'word_p1', 'word_p2',
                     'pos_n2', 'pos_n1', 'pos', 'pos_p1', 'pos_p2','c-n2','c-n1']


    train_sentences = conll_reader.read_sentences(train_corpus)

    print("Extracting the features...")
    X_dict, y_classes = extract_features(train_sentences, w_size, feature_names)
    ...
    y_encoded, dict_classes, inv_dict_classes = encode_classes(y_classes)
    ...
    model = classifier.fit(X, y_encoded)
    ...
    y_test = [inv_dict_classes[i] if i in y_classes else 0 for i in y_test_symbols]
    y_test_predicted = classifier.predict(X_test)
